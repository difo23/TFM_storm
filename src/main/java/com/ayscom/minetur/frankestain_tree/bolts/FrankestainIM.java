...
public class FrankestainIM extends BaseRichBolt {
    ...
    public void execute(Tuple tuple) {
       ...
       // DBObject[] docs = new DBObject[0];
        ...
        Vector<DBObject> xdrs_im = new Vector<DBObject>();
        Date pm = new Date();
        ...
        try {
            if (tuple.getSourceStreamId().equals("franksearchpm_bolt")) {
                ...
                DBObject[] dbObject = (DBObject[]) JSON.parse(xdrim.toString());
...
*/
                ...
                System.out.println("--------fecha arreglo im "+dbObject[0].get("_id") );
        ...
        if (!xdrs_im.isEmpty()) {
            System.out.println("*************** Este grupo posee " + xdrs_im.size() + " xdrs iniciamos frankson. Vale! ***********");
            this.collector.emit("franksearchpm_bolt", tuple, new Values(obj_user, user.get(1), JSON.serialize(xdrs_im)));
            this.collector.ack(tuple);

        } else {
            System.out.println("*************** No existen xdrs corralados por intervalo medio. Vale! ***********");
